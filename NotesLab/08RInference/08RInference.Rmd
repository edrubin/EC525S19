...
sim_analyze <- function(data) {
  # Conventional SEs
  result_ols <- lm_robust(y ~ x, data = data, se_type = "classical") %>%
    tidy() %>% filter(term == "x") %>% select(2:5)
  # Cluster-robust SEs
  result_cl <- lm_robust(y ~ x, data = data, clusters = g) %>%
    tidy() %>% filter(term == "x") %>% select(2:5)
  # Bind results together and add column for standard errors
  results_df <- bind_rows(result_ols, result_cl) %>%
    mutate(type = c("conventional", "clustered"))
  # Return results
  return(results_df)
}
```
...
.hi-slate[Comparing standard errors]

```{R, plot-sim-se, echo = F, eval = T}
ggplot(data = sim_df, aes(x = std.error, fill = type)) +
...
.hi-slate[Comparing _t_ statistics]

```{R, plot-sim-tstat, echo = F, eval = T}
ggplot(data = sim_df, aes(x = statistic, fill = type)) +
geom_density(color = NA, alpha = 0.9) +
geom_hline(yintercept = 0) +
xlab("t Statistic") +
ylab("Density") +
...
`sim_df %>% group_by(type) %>% summarize(mean(p.value < 0.05))`
