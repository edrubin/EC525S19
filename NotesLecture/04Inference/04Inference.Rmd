...
However, we can make statistical inferences about the population using samples.

--

.hi-slate[Important] The issue/topic of *statistical inference* is separate from *causality*.
--
 Regardless of the interpretation of a coefficient, it has a sampling distribution.
...
$$
\begin{align}
  \hat{\beta} = \left( \text{X}^\prime \text{X} \right)^{-1} \text{X}^\prime \text{y} = \left( \sum_i \text{X}_{i}\text{X}_{i}' \right)^{-1}
\end{align}
$$
.footnote[.pink[†] *MHE* presents a method-of-moments motivation for this derivation. You've also seen others, _e.g._, minimizing MSE of $\text{Y}_{i}$ given $\text{X}_{i}$.]
