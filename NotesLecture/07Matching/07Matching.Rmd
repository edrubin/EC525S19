...
∴ $\left( \text{Y}_{0i},\,\text{Y}_{1i} \right) \ci \text{D}_{i}|\text{X}_{i} \implies \left( \text{Y}_{0i},\,\text{Y}_{1i} \right) \ci \text{D}_{i}|p(\text{X}_{i})$ ✔
---
layout: true
# Propensity-score methods
...
.attn[Caution] Logit can hid a violation because it forces $0 < \hat{p}(\text{X}_{i}) < 1$.

--

.note[Common practice] To empirically enforce overlap:
