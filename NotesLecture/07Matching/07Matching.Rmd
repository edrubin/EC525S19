...
# Generate data
overlap <- tibble(
  x = runif(n = n, min = 10, max = 90),
  e = rnorm(n),
  p = (x + e) / 100,
  trt = rbinom(n = n, size = 1, prob = p),
)
overlap$p_hat <- glm(trt ~ x, data = overlap, family = "binomial")$fitted.values
# Plot
ggplot(
  data = overlap,
  aes(
...
.note[Note] Overlap in 1 dimension does not guarantee in 2D
...
coord_equal() +
scale_fill_manual("Prop. score", values = c("grey10", "white")) +
